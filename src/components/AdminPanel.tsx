...
import { MARCA, MINIMO_MAYORISTA, BASE_PATH, FONDOS, PROMOS, TEL_MAYORISTA_VISIBLE } from "@/lib/config";
...
/* Bloque de fotos (generales del producto, no por color) + colores disponibles
   (solo etiquetas, sin foto propia — elegir color no cambia ninguna imagen). */
function BloqueFotosColores() {
  ...
  function agregar() {
    setColores((prev) => [...prev, { hex, n: nombre.trim() || "Color" }]);
    setNombre("");
    setHex("#c4b0dd");
    setAbierto(false);
  }

  function Pastilla({ hex, n, onQuitar }: { hex: string; n: string; onQuitar: () => void }) {
    return (
      <span className="flex items-center gap-1.5 rounded-full border border-borde bg-superficie py-1 pl-1 pr-2 text-xs">
        <span className="h-5 w-5 rounded-full border border-borde" style={{ backgroundColor: hex }} />
        {n}
        <button
          type="button"
          onClick={onQuitar}
          className="ml-0.5 text-tenue transition-colors hover:text-[#c0392b]"
          aria-label={`Quitar ${n}`}
        >
          ×
        </button>
  ...
  return (
    <div className="rounded-xl border border-borde bg-superficie/40 p-4 sm:col-span-2">
      ...
      <p className="mt-0.5 text-xs text-tenue">
        Subí las fotos del producto y marcá en qué colores lo tenés. La foto no cambia según el color — el cliente elige el color aparte al hacer el pedido.
      </p>

      {/* FOTOS DEL PRODUCTO */}
      <div className="mt-4 rounded-lg border border-borde bg-background p-3.5">
        <p className="mb-3 text-[11px] font-semibold text-foreground">Fotos del producto</p>
      ...
      {/* COLORES DISPONIBLES */}
      <div className="mt-3 rounded-lg border border-borde bg-background p-3.5">
        <p className="mb-3 text-[11px] font-semibold text-foreground">Colores disponibles</p>
        <div className="flex flex-wrap gap-2">
          {colores.map((c, i) => (
            <Pastilla
              ...
              hex={c.hex}
              n={c.n}
              onQuitar={() => setColores((prev) => prev.filter((_, j) => j !== i))}
            />
          ))}
        </div>
            ...
            <div className="space-y-5">
              <div className="rounded-xl border border-borde bg-background p-6">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <Campo label="Nombre" placeholder="Ej: Conjunto Mujer Oversize" />
                  ...
                  <table className="w-full min-w-[560px] text-left text-sm">
                    <thead className="border-b border-borde text-xs uppercase tracking-wide text-tenue">
                      <tr>
                        ...
                        <th className="py-2.5 pr-3 font-medium">Mayorista</th>
                    ...
                    <tbody className="divide-y divide-borde">
                      {[...productos].sort((a, b) => (b.sinStock ? 1 : 0) - (a.sinStock ? 1 : 0)).slice(0, 8).map((p) => (
                        <tr key={p.slug}>
                          ...
                          <td className="py-2.5 pr-3 font-semibold">{precio(p.precioMayorista)}</td>
                          <td className="py-2.5 pr-3">
                            {p.sinStock ? (
            ...
            <div className="space-y-5">
              <div className="rounded-xl border border-borde bg-background p-6">
                ...
                <div className="relative mb-1.5 flex h-64 max-w-sm overflow-hidden rounded-lg border border-borde bg-superficie sm:h-72">
                  ...
                  <div className="relative flex h-full w-[45%] shrink-0 items-center justify-center overflow-hidden bg-acento/10">
                    <span className="text-[10px] font-medium uppercase tracking-widest text-acento/60">Sin foto</span>
                    <span className="absolute bottom-2 right-2 cursor-not-allowed rounded-none bg-background/90 px-2.5 py-1 text-[11px] font-semibold text-acento">🖼️ Subir foto</span>
                  </div>
                </div>
                <p className="mb-4 flex flex-wrap items-center gap-2 text-[11px] text-tenue">
                  <span className="rounded-full bg-acento/15 px-2.5 py-1 text-xs font-semibold text-acento">📐 Medida exacta: 812 × 850 px</span>
                ...
                <div className="grid gap-4">
                  <Campo label="Etiqueta de arriba" valor="Conjuntos · Venta por mayor" />
                  <Campo label="Título" valor="Conjuntos al por mayor" />
                  <Campo label="Bajada" area valor="Elegí los modelos, armá tu pedido por mayor y lo cerramos por WhatsApp — con envíos a todo el país." />
                </div>
                <Bloqueado />
              </div>
              ...
              <div className="rounded-xl border border-borde bg-background p-6">
                ...
                <div className="grid gap-5 sm:grid-cols-2">
                  ...
                  <div>
                    ...
                    <div className="relative mb-1.5 flex aspect-[1894/777] w-full items-center justify-center overflow-hidden rounded-lg border border-borde bg-acento/10">
                      <span className="text-[10px] font-medium uppercase tracking-widest text-acento/60">Sin foto</span>
                      <span className="absolute bottom-2 right-2 cursor-not-allowed rounded-none bg-background/90 px-2.5 py-1 text-[11px] font-semibold text-acento">🖼️ Subir foto</span>
                    </div>
                  ...
                  <div>
                    ...
                    <div className="relative mb-1.5 flex aspect-square w-full max-w-[240px] items-center justify-center overflow-hidden rounded-lg border border-borde bg-acento/10">
                      <span className="text-[10px] font-medium uppercase tracking-widest text-acento/60">Sin foto</span>
                      <span className="absolute bottom-2 right-2 cursor-not-allowed rounded-none bg-background/90 px-2.5 py-1 text-[11px] font-semibold text-acento">🖼️ Subir foto</span>
                    </div>
              ...
              <div className="rounded-xl border border-borde bg-background p-6">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <Campo label="Etiqueta de arriba" valor="Tu ritmo, tu actitud" />
                  ...
                  <div className="sm:col-span-2">
                    <Campo label="Texto" area valor={`Conjuntos al mejor precio y calidad. Accedé a precio mayorista desde solo ${MINIMO_MAYORISTA} prendas.`} />
                  </div>
                </div>
                <p className="mt-3 rounded-lg bg-acento/10 px-3 py-2 text-[11px] text-tenue">
          ...
          {vista === "config" && (
            <div className="rounded-xl border border-borde bg-background p-6">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <Campo label="WhatsApp" valor={TEL_MAYORISTA_VISIBLE} />
                <Campo label="Envíos" valor="A todas las provincias de Argentina" />
                <Campo label="Rubro" valor="Indumentaria · Mayorista" />
