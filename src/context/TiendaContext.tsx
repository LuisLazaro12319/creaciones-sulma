...
const CLAVE_CARRITO = "sulma.carrito";
...
export function TiendaProvider({ children }: { children: React.ReactNode }) {
  ...
  // Tienda 100% mayorista: no hay modo minorista para elegir.
  const precioDe = useCallback(
    (producto: Producto) => producto.precioMayorista,
    [],
  );
  ...
  const faltanParaMayorista = Math.max(0, MINIMO_MAYORISTA - unidades);
